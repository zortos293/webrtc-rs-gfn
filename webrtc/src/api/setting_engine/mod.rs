...
pub struct SettingEngine {
    ...
    /// Allow provisional SSRC handling for GFN compatibility.
    /// When enabled, undeclared SSRCs will be routed to existing video tracks
    /// even when multiple media sections exist (without requiring MID extensions).
    pub(crate) allow_provisional_ssrc: bool,
}
...
impl SettingEngine {
    ...
    /// set_allow_provisional_ssrc enables provisional SSRC handling for GFN compatibility.
    /// 
    /// When enabled, undeclared SSRCs (from mid-stream resolution changes) will be
    /// routed to existing video tracks without requiring MID RTP header extensions.
    /// This mimics libsrtp2's provisional stream feature used by the official GFN client.
    /// 
    /// Default: false (standard WebRTC behavior requiring MID extensions)
    pub fn set_allow_provisional_ssrc(&mut self, allow: bool) {
        self.allow_provisional_ssrc = allow;
    }
